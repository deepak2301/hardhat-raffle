  ...
  : describe("Raffle Unit Tests", async function () {
      ...
      describe("Enter Raffle", async () => {
        ...
        it("tests if the emit is working fine", async () => {
          await expect(raffle.enterRaffle({ value: raffleEntryFee })).to.emit(
            raffle,
            "RaffleEnter"
          ) //* Emit "RaffleEnter from raffle contract"
        })
        it("Should not allow entrance if raffleState is clculating ", async () => {
          ...
          await raffle.performUpkeep([] /*  pass a call data "Empty array" */)
          await expect(
